module.exports = `
menu.btn.ranked Kilpailullinen
menu.btn.shop Kauppa
menu.btn.social Hubi
menu.btn.maps Pelit
menu.btn.mods Modit
menu.btn.settings Asetukset
menu.btn.customize Mukauta
menu.btn.invite Kutsu
menu.btn.invite.clicked Kopioitu
menu.btn.join Liity
menu.btn.host Luo Peli
menu.btn.browser Palvelimet
generic.wait Odota...
generic.loading LADATAAN...
generic.none Ei Mitään
generic.connecting YHDISTETÄÄN...
generic.select Valitse
generic.delete Poista
generic.save Tallenna
generic.load Lataa
generic.search Etsi
generic.leave Poistu
generic.default Oletus
generic.submit Lähetä
generic.create Luo
generic.random Satunnainen
generic.failed Epäonnistui
generic.success Onnistui!
generic.name Nimi
generic.level Taso
generic.score Pisteet
generic.off Pois Päältä
generic.defeat TAPPIO
generic.victory VOITTO
generic.matchover PELI PÄÄTTYNYT
generic.matchaband PELI HYLÄTTY
generic.follow Seuraa
generic.unfollow Lopeta Seuraaminen
generic.followers Seuraajat
generic.following Seurataan
generic.games Pelejä
generic.wins Voitot
generic.lose Häviöt
generic.clan Klaani
generic.games.played Pelatut Pelit
generic.games.won Voitetut Pelit
generic.games.lost Hävityt Pelit
generic.timep Pelattu Aika
generic.kills Tapot
generic.deaths Kuolemat
generic.kdr KDR
generic.time Aika
generic.logout Kirjaudu Ulos
generic.logout.sessions Kirjaudu ulos kaikista istunnoista
generic.username Syötä käyttäjätunnus
generic.password Syötä salasana
generic.register Rekisteröidy
generic.login Kirjaudu Sisään
generic.accuracy Tarkkuus
player.waiting odotetaan pelaajia
player.died Kuolit
player.killed Sinut Tappoi
player.killstreak {0} on a {1} Kill Streak
player.inactive Heitetty ulos toimettomuuden vuoksi
player.disconnect YHTEYS KATKENNUT
timer.end Seuraava peli alkaa {0}:n kuluttu
leaderboard.empty Tyhjä Serveri
popup.checkpoint Tallennuspiste!
app.play KLIKKAA PELATAKSESI
app.spectate KLIKKAA KATSELLAKSESI
app.play.mobile NAPAUTA PELATAKSESI
app.spectate.mobile NAPAUTA KATSELLAKSESI
app.play.controller PAINA START PELATAKSESI
app.spectate.controller PAINA START KATSELLAKSESI
purchase.error Tapahtui virhe
purchase.error.click paina tästä</a> ja kokeile uudelleen!
purchase.error.limit Olet saattanut ylittää päivittäisen tapahtumarajan
settings.reset Resetoi
settings.reset.confirm Haluatko varmasti resetoida kaikki asetuksesi?
settings.import Tuo
settings.export Vie
settings.none Asetuksia ei löydetty
settings.backup Varmuuskopiointiasetukset
settings.controls.change Näppäinasetukset
settings.controls.press Paina mitä tahansa näppäintä
settings.local.header Lokalisointi <span id="requiresRestart"><span style="color: #eb5656">*</span> vaatii uudelleenkäynnistyksen</span>
settings.local.region Oletusalue
settings.local.lang Kieli
settings.network.header Verkko
settings.network.netRerouting Automaattinen Reititys (Vaikuttaa Viiveeseen)
settings.network.netRate Netin Taajuus (Hz)
settings.network.lagComp Viiveen Korjaus
settings.network.optNet Optimoitu Nettiyhteys
settings.popup.header Ponnahdusikkunat
settings.experimental.header Kokeelliset Asetukset
settings.experimental.holiday Loma-Efektit
settings.experimental.aimFreezeFix Tähtäyksen Jähmettymisen Korjaus
settings.experimental.mouseFlickFix Hiiren Nopean Liikahtamisen Korjaus
settings.experimental.shaderRendering Instance Rendering
settings.experimental.mouseAccel Hiiren Kiihtyvyys
settings.experimental.tbInterp TB Interpolaatio
settings.client.header Client
settings.client.unlimitedFrames Rajaton FPS
settings.client.gameCap Tallenna Ottelu
settings.client.experimental Kokeelliset ominaisuudet
settings.client.discordRPC Discord RPC
settings.client.resourceSwapper Resurssien Vaihtaja
settings.client.clearCache Tyhjennä välimuisti
settings.client.resetSettings Resetoi Ohjelman Asetukset
settings.client.acceleratedCanvas Kiihdytetty Grafiikanlataus
settings.client.angleBackend Palvelinpään ANGLE Grafiika
settings.client.colorProfile Väriprofiili
settings.client.resourcePacks Resurssipakkaukset
settings.dev.header Kehittäjäasetukset
settings.dev.forceUpdate Pakota Skinien Päivitys
settings.dev.forceRegen Regeneroi Pelaaja
settings.dev.extraBan Lisätietoa Porttikiellosta
settings.chat.header Chatti
settings.chat.chat Näytä Chatti
settings.chat.chat.all Aina
settings.chat.chat.focus Kun Kohdistettu
settings.chat.messages Näytä pelaajien viestit
settings.chat.unboxings Näytä skinien saamisviestit
settings.chat.chatOp Chatin läpinäkyvyys
settings.chat.chatBGOp Chatin taustan läpinäkyvyys
settings.chat.chatHeight Chatin korkeus
settings.chat.chatTimer Chatin Ajastin
settings.chat.chatTextOutline Chatin ääriviivat
settings.chat.chatFilter Kirosanasuodatin
settings.quality.header Suorituskyky
settings.quality.updRate Maksimikuvataajuus
settings.quality.rendDst Renderöintietäisyys
settings.quality.res Resoluutio
settings.quality.part Partikkelit
settings.quality.mapDet Kartan Yksityiskohdat
settings.quality.noTex Ei Tekstuureja
settings.quality.partdist Partikkelin Etäisyys
settings.quality.renderdist Näköetäisyys
settings.quality.reflection Heijastuksen Laatu
settings.quality.antiAlias Reunojen Pehmennys
settings.quality.shadows Varjot
settings.quality.softShad Heikot Varjot
settings.quality.highResShad Korkealaatuiset Varjot
settings.quality.shadowsDynamic Dynaamiset Varjot
settings.quality.trails Luotien Vanat
settings.quality.yourTrails Luotiesi Vanat
settings.quality.mflash Piipun Välähdys
settings.quality.flap Tarkkuuskiväärin Läppä
settings.quality.lowSpec Alhaiset Asetukset
settings.quality.ambient Ympäristön Varjostus
settings.quality.ambientOld Vanha Varjostus (Kun Mahdollista)
settings.quality.textureAnim Tekstuurien Animaatiot
settings.quality.objectAnim Objektien Animaatiot
settings.quality.screenShake Näytön Ravistus
settings.quality.weaponShine Aseen Kiiltäminen
settings.quality.rendGamma Gamma
settings.quality.lighting Valaistus
settings.quality.showExplo Näytä Räjähdykset
settings.quality.aspectRatio Kuvasuhde
settings.post.header Jälkikäsittely
settings.post.post Jälkikäsittely
settings.post.bloom Hehku
settings.post.bloomThreshold Hehkun Kynnysarvo
settings.post.bloomStrength Hehkun Voimakkuus
settings.post.bloomRadius Hehkun Säde
settings.post.ssao SSAO
settings.post.ssaoRadius SSAO Säde
settings.interface.hitInd Näytä Osumien Ilmoitukset
settings.interface.hitColor Osuman Ilmoituksen Väri
settings.interface.dmg Näytä Vahinko
settings.interface.dmgCol Vahingon Väri
settings.interface.critCol Kriittisen Vahingon Väri
settings.interface.header Käyttöliittymä
settings.interface.scale Käyttöliittymän Koko
settings.interface.theme Pelin Teema
settings.interface.ui Näytä Käyttöliittymä
settings.interface.scoreboard Käytä Vanhaa Tulostaulukkoa
settings.interface.oldBrowser Käytä Vanhaa Selainta
settings.interface.rarityanim Kytke Harvinaisuuksien Animaatiot Pois
settings.interface.showPfp Näytä Profiilikuvat
settings.interface.showTradeMsg Näytä Vaihtokauppojen Viestit
settings.interface.hideNonTrade Piilota Ei-Vaihdettavat Skinit
settings.interface.hp Dynaamiset HP-pisteet
settings.interface.dmgScale Vahingon Laajuus
settings.interface.killf Näytä Tappolista
settings.interface.ping Näytä Viive
settings.interface.net-stats Näytä Verkon Tilasto
settings.interface.fps Näytä FPS
settings.interface.deaths Näytä Kuolemat
settings.interface.kd Näytä K/D
settings.interface.kills Näytä Tapot
settings.interface.streak Näytä Tappoputki
settings.interface.score Näytä Pisteet
settings.interface.spectators Näytä Katselijat
settings.interface.speed Näytä Liikkumisnopeus
settings.interface.speedOffX Nopeuslaskurin Sijainti X
settings.interface.speedOffY Nopeuslaskurin Sijainti Y
settings.interface.speedOpacity Nopeuslaskurin Läpinäkyvyys
settings.interface.speedScale Nopeuslaskurin Koko
settings.interface.speedColor Nopeuslaskurin Väri (Tämänhetkinen)
settings.interface.speedColorMax Nopeuslaskurin Väri (Maksimi)
settings.interface.hideNames Nimimerkkien Näkyvyys
settings.interface.hideNames.team Ainoastaan Tiimi
settings.interface.hideNames.enemy Ainoastaan Viholliset
settings.interface.hideNames.all Kaikki
settings.interface.nametagStyle Nimimerkkien Tyyli
settings.interface.nametagOpac Nimimerkkien Läpinäkyvyys
settings.interface.nametagScale Nimimerkkien Koko
settings.interface.nametagStyle.health Ainoastaan HP
settings.interface.nametagStyle.name Ainoastaan Nimi
settings.interface.nametagStyle.nameLvl Ainoastaan Nimi & Taso
settings.interface.nametagStyle.all Kaikki
settings.interface.healthNum HP-Pisteet Nimimerkissä
settings.interface.healthColT Oman Tiimin Nimimerkkien Väri
settings.interface.healthColE Vihollistiimin Nimierkkien Väri
settings.interface.xpBarCol XP-palkin Väri
settings.interface.xpBarCol2 XP-palkin Väri 2
settings.interface.xpBarOpac XP-palkin Läpinäkyvyys
settings.medals.header Mitalit 
settings.medals.show Näytä Mitalit 
settings.medals.classic Vanhat Mitalit 
settings.medals.sound Soita Mitalin Ääni 
settings.medals.scale Mitalin Koko 
settings.medals.xOff Mitalin Sijainti X 
settings.medals.yOff Mitali Sijainti Y 
settings.crosshair.header Crosshair
settings.crosshair.type Tyyppi
settings.crosshair.type.1 Dynaaminen
settings.crosshair.type.2 Muodot
settings.crosshair.type.3 Kerroksittainen
settings.crosshair.type.4 Kuva
settings.crosshair.type.5 Eritelty Risti
settings.crosshair.style Tyyli
settings.crosshair.style.0 Risti
settings.crosshair.style.1 Ontto Ympyrä
settings.crosshair.style.2 Kiinteä Ympyrä
settings.crosshair.style.3 Ontto Neliö
settings.crosshair.style.4 Kiinteä Neliö
settings.crosshair.image Kuva
settings.crosshair.image.paste Tähtäimen Kuva
settings.crosshair.image.width Kuvan Leveys
settings.crosshair.image.height Kuvan Korkeus
settings.crosshair.always Näytä Aina
settings.crosshair.color Väri
settings.crosshair.shadow.color Varjon Väri
settings.crosshair.shadow.thickness Varjon Paksuus
settings.crosshair.size Koko
settings.crosshair.thickness Paksuus
settings.crosshair.gap Etäisyys
settings.crosshair.dot Piste
settings.crosshair.opacity.use Käytä Mukautettua Läpinäkyvyyttä
settings.crosshair.opacity.custom Mukautettu Läpinäkyvyys
settings.crosshairm.header Crosshair (Puukkoa Heitettäessä)  
settings.crosshairm.type Tyyppi 
settings.crosshairm.type.1 Muodot 
settings.crosshairm.type.2 Kuva 
settings.crosshairm.type.3 Eritelty Risti
settings.crosshairm.style Tyyli 
settings.crosshairm.style.0 Risti 
settings.crosshairm.style.1 Ontto Ympyrä 
settings.crosshairm.style.2 Kiinteä Ympyrä 
settings.crosshairm.style.3 Ontto Neliö 
settings.crosshairm.style.4 Kiinteä Neliö  
settings.crosshairm.image Kuva  
settings.crosshairm.image.paste Crosshairin Kuva  
settings.crosshairm.image.width Kuvan Leveys  
settings.crosshairm.image.height Kuvan Korkeus  
settings.crosshairm.always Näytä Aina  
settings.crosshairm.color Väri  
settings.crosshairm.shadow.color Varjon Väri  
settings.crosshairm.shadow.thickness Varjon Paksuus 
settings.crosshairm.size Koko 
settings.crosshairm.thickness Paksuus 
settings.crosshairm.gap Etäisyys 
settings.crosshairm.dot Piste 
settings.crosshairm.opacity.use Käytä Mukautettua Läpinäkyvyyttä 
settings.crosshairm.opacity.custom Mukautettu Läpinäkyvyys
settings.hitmarker.header Näytä Osuma
settings.hitmarker.color Väri
settings.hitmarker.kcolor Tappoväri
settings.hitmarker.thickness Paksuus
settings.hitmarker.opacity Läpinäkyvyys
settings.hitmarker.leng Korkeus
settings.hitmarker.spacing Väli
settings.hitmarker.animD Animaation Koko
settings.hitmarker.animS Animaation Nopeus
settings.hitmarker.hitm Näytä
settings.hitmarker.hitmFad Häivytyksen Nopeus
settings.hitmarker.custom Osuman Kuva
settings.hitmarker.custom.paste Osuman Kuvan URL
settings.onKillS.style.0 Oletus
settings.onKillS.style.1 Vähennä Resoluutiota
settings.onKillS.style.2 Vähennä Näkökenttää
settings.onKillS.style.3 Lisää Näkökenttää
settings.serverb.header Palvelinselain
settings.serverb.oldBrowser Vanha Selain
settings.serverb.defaultRegionOnly Vain Oletusalue
settings.gameprivacy.header Pelaamisen Yksityisyys
settings.gameprivacy.streamer Striimaajatila
settings.gameprivacy.anon Anonyymi Tila
settings.gameprivacy.hideGameTitle Piilota Pelin Otsikko
settings.gameprivacy.showRegion Näytä Alueen Tiedot
settings.gameprivacy.verified Näytä Verified-Merkki
settings.gameprivacy.premium Näytä Premium-Merkki
settings.gameprivacy.partner Näytä Yhteistyö-Merkki
settings.gameprivacy.rank Näytä Kilpailullisen Tasomerkki
settings.gameprivacy.custom Näytä Kustomoitu Merkki
settings.gameplay.header Pelaaminen
settings.gameplay.gpSetts Peliasetukset
settings.gameplay.sensitivityX Herkkyys (X)
settings.gameplay.sensitivityY Herkkyys (Y)
settings.gameplay.aimsensX Tähtäyksen Herkkyys (Y)
settings.gameplay.aimsensY Tähtäyksen Herkkyys (X)
settings.gameplay.challenge Haastava Moodi
settings.gameplay.invert Peilaa Y-Akseli
settings.gameplay.onkillS Tapossa
settings.gameplay.scrollDir Vierityssuunta
settings.controller.header Ohjain
settings.controller.sensitivityX Herkkyys(X)
settings.controller.disableCons Poista Ohjaimet Käytöstä
settings.controller.sensitivityY Herkkyys (Y)
settings.controller.aimsensX Tähtäyksen Herkkyys (X)
settings.controller.aimsensY Tähtäyksen Herkkyys (Y)
settings.controller.deadZoneL Kuollut Alue (Vasen)
settings.controller.deadZoneR Kuollut Alue (Oikea)
settings.controller.triggerThres Liipaisimen Kynnysarvo
settings.controller.vibration Tärinä
settings.controller.gradSpeed Asteittainen Nopeus
settings.controller.invert Peilaa Y-Akseli
settings.mobile.header Puhelin
settings.mobile.sensitivityX Herkkyys (X)
settings.mobile.sensitivityY Herkkyys (Y)
settings.mobile.aimsensX Tähtäyksen Herkkyys (X)
settings.mobile.aimsensY Tähtäyksen Herkkyys (Y)
settings.mobile.vibration Tärinä
settings.mobile.gradSpeed Asteittainen Nopeus
settings.mobile.invert Peilaa Y-Akseli
settings.audio.header Äänet
settings.audio.sound Pää-äänenvoimakkuus
settings.audio.ambient Ympäristön Äänenvoimakkuus
settings.audio.voice VC-Äänenvoimakkuus
settings.audio.input Äänen/Mikrofonin Lähde
settings.audio.mic Mikrofonin Äänenvoimakkuus
settings.audio.micquality Mikrofonin Äänenlaatu
settings.audio.weps Aseen Äänenvoimakkuus
settings.audio.player Hahmojen Äänenvoimakkuus
settings.audio.skin Kosmetiikan Äänenvoimakkuus
settings.audio.ui Käyttöliittymän Äänenvoimakkuus
settings.audio.asset Assettien Äänenvoimakkuus
settings.audio.dialogue Toiminnan Äänenvoimakkuus
settings.viewmodel.header Näkymä
settings.viewmodel.vmSetts Tarkastele Näkymän Asetuksia
settings.viewmodel.fov Näkökenttä
settings.viewmodel.fps Aseen Näkökenttä
settings.viewmodel.bobbing Aseen Heiluminen
settings.viewmodel.leaning Aseen Kaltevuus
settings.viewmodel.show.primary Näytä Ensisijainen Ase
settings.viewmodel.show.secondary Näytä Toissijainen Ase
settings.viewmodel.roundArms Pyöreät Kädet
settings.viewmodel.show.melee Näytä Melee
settings.viewmodel.show.ads Piilota Ase Tähdättäessä
settings.viewmodel.toggleAim Vaihda Tähtäys
settings.viewmodel.show.hands Näytä Kädet
settings.viewmodel.leftHand Vasenkätinen
settings.viewmodel.aimAnim Aseen Tähtäysanimaatio
settings.viewmodel.rot Aseen Rotaatio
settings.viewmodel.xOff Aseen Sijainti (X)
settings.viewmodel.yOff Aseen Sijainti (Y)
settings.viewmodel.zOff Aseen Sijainti (Z)
settings.viewmodel.weapSwapY Aseen Vaihto (Y)
settings.viewmodel.weapReloadY Aseen Lataus (Y)
settings.viewmodel.yOffADS Aseen Sijainti Tähdätessä (Y)
settings.viewmodel.adsFovMlt FOVin Voimakkuus Tähdätessä
settings.viewmodel.toggleADS Vaihda Aseen Tähtäysasento
settings.editing.header Editoi
settings.editing.health.high HUD Elämiä Paljon
settings.editing.health.low HUD Elämiä Vähän
settings.editing.speedLines Vauhtiviivat
settings.editing.spdLinesCol Vauhtiviivojen Väri
settings.editing.score.color Popup-Pisteiden Väri
settings.editing.score.show Näytä Popup-Pisteet
settings.editing.score.scale Popup-Pisteiden Koko
settings.editing.score.shadow Popup-Pisteiden Varjostus
settings.editing.scoreOffX Popup-Pisteiden X-Akselissa Sijainti
settings.editing.scoreOffY Popup-Pisteiden Y-Akselissa Sijainti
settings.editing.prog.color Edistymispalkin Väri
settings.editing.prog.opacity Edistymispalkin Läpinäkyvyys
settings.editing.prog.scale Edistymispalkin Koko
settings.editing.prog.shadow Edistymispalkin Varjostus
settings.editing.progOffX Edistymispalkin Sijainti (X)
settings.editing.progOffY Edistymispalkin Sijainti (Y)
settings.editing.progRot Edistymispalkin Rotaatio
settings.editing.progm.color Latautumispalkin Väri
settings.editing.progmo.color Ylilatautumisen Väri
settings.editing.progm.opacity Latautumispalkin Läpinäkyvyys
settings.editing.progm.scale Latautumispalkin Koko
settings.editing.progm.shadow Latautumispalkin Varjo
settings.editing.progmOffX Latautumispalkin Sijainti (X)
settings.editing.progmOffY Latautumispalkin Sijainti (Y)
settings.editing.progmRot Latautumispalkin Rotaatio
settings.editing.saturation Saturaatio (Pelinsisäinen)
settings.editing.menuSaturation Saturaatio (UI)
settings.editing.hue Värisävy (Pelinsisäinen)
settings.editing.menuHue Värisävy (UI)
settings.editing.vignette Vinjetti
settings.editing.feedLimit Tappolistan Raja
settings.editing.depth Syvyyskartta
settings.editing.green Väriavain
settings.editing.trailcol Luodin Vanan Väri
settings.editing.ropecol Heittokoukun Köyden Väri
settings.editing.tracercol Luodin Tracerit
settings.editing.tracer Luodin Tracerit
settings.editing.tracer.offset Tracerin Sijainti
settings.shaders.header Varjostimen Syöte
settings.mods.header Kustomointi
settings.mods.load Lataa Modeja
settings.mods.logo Hyväksy Logon Muutokset
settings.mods.auto Lataa Modeja Automaattisesti
settings.mods.borders Tähtäimen Reunat
settings.mods.borderCol Tähtäimen Reunat
settings.mods.borderOpac Tähtäimen Reunojen Läpinäkyvyys
settings.mods.scope Tähtäimen Kuva
settings.mods.scope.paste Tähtäimen Kuvan URL
settings.mods.scope.opacity Tähtäimen Läpinäkyvyys
settings.mods.scope.width Tähtäimen Kuvan Leveys
settings.mods.scope.height Tähtäimen Kuvan Korkeus
settings.mods.dot Reticle-Kuva
settings.mods.dot.width Reticle-Kuvan Leveys
settings.mods.dot.height Reticle-Kuvan Korkeus
settings.mods.dot.paste Reticlen URL
settings.mods.endmessage Ottelun Loppuviesti
settings.mods.profile Profiilikuva
settings.mods.profile.paste Profiilikuvan URL
settings.mods.ammo Ammuskuvakkeen Kuva
settings.mods.ammo.paste Ammuskuvakkeen Kuvan URL
settings.mods.kills Tappokuvakkeen Kuva
settings.mods.kills.paste Tappokuvakkeen Kuvan URL
settings.mods.deaths Kuolemakuvakkeen Kuva
settings.mods.deaths.paste Kuolemakuvakkeen Kuvan URL
settings.mods.streak Tappoputkikuvakkeen Kuva
settings.mods.streak.paste Tappoputkikuvakkeen Kuvan URL
settings.mods.timer Ajastinkuvakkeen Kuva
settings.mods.timer.paste Ajastiskuvakkeen Kuvan URL
settings.mods.usedamage Käytä Vahingon Overlayta
settings.mods.damage Vahingon Overlayn Kuva
settings.mods.damage.paste Vahingon Overlayn Kuvan URL
settings.mods.overlay Pelin Overlayn Kuva
settings.mods.overlay.paste Pelin Overlayn Kuvan URL
settings.social.region Alueen Tilastot
settings.social.bg Taustakuva
settings.social.bg.paste Taustakuvan URL
windows.settings.header Peliasetukset
windows.mail.header Krunker-Posti
windows.servers.header Palvelimet
windows.servers.none Liityttäviä Pelejä Ei Löytynyt...
windows.servers.host + Luo Peli
windows.servers.hide Piilota Täynnä Olevat
windows.servers.priority Alueen Prioriteetti
windows.servers.online {0} Pelissä
windows.servers.social {0} Hubissa
windows.servers.editor {0} Editorissa
windows.loadout.header Vaihda Varustusta
windows.loadout.class Hahmo
windows.loadout.primary Ensisijainen
windows.loadout.primary.attach Lisävarusteet
windows.loadout.primary.reticle Reticle
windows.loadout.primary.scope Tähtäin
windows.loadout.secondary Toissijainen
windows.loadout.secondary.skin Toissijainen Skini
windows.loadout.melee Melee
windows.loadout.hat Hattu
windows.loadout.back Keho
windows.loadout.waist Vyötärö
windows.loadout.shoe Kengät
windows.loadout.face Naama
windows.loadout.spray Spray
windows.loadout.color.skin Ihonväri
windows.loadout.color.chat Nimen Väri Chatissa
windows.loadout.color.hair Hiusten Väri
windows.loadout.dye Värjäys
windows.loadout.pet Lemmikki
windows.loadout.wrist Ranne
windows.loadout.stat Tappokortin Tilastot
windows.loadout.flag Maan Lippu
windows.loadout.badge Kustomoitu Merkki
windows.mods.drop tiputa mod-tiedosto tähän tai klikkaa
windows.mods.paste Liitä Modin URL
windows.mods.load Lataa Modi
windows.mods.none Modeja ei löydetty
windows.mods.header Modihallinta
windows.mods.guide Modiopas
windows.mods.list Modilista
windows.mods.load.header Lataa Modit
windows.mods.publish Julkaise Modi
windows.mods.viewer Tarkastelu
windows.account.header Tili
windows.account.recover Jos haluat palauttaa tilin, ota yhteyttä <a>recovery@yendis.ch</a>
windows.class.header Valitse Hahmo
windows.controls.header Vaihda Näppäinasetuksia
windows.controls.forward Eteenpäin
windows.controls.backward Taaksepäin
windows.controls.left Vasemmalle
windows.controls.right Oikealle
windows.controls.reload Lataa Ase
windows.controls.aim1 Tähtääminen
windows.controls.shoot1 Ampuminen
windows.controls.inspect Tutkiminen
windows.controls.spray Spray
windows.controls.sprayWheel Spray-Pyörä (Pidä Painettuna)
windows.controls.jump Hyppääminen
windows.controls.crouch Kyyristyminen
windows.controls.prim Ensisijainen Ase
windows.controls.swap Toissijainen Ase
windows.controls.melee Melee-Ase
windows.controls.equip Varusteet
windows.controls.chat Chatti
windows.controls.voice Puheääni
windows.controls.list Pelaajalista
windows.controls.board Tulostaulukko
windows.controls.interact Vuorovaikutus
windows.controls.interactSec Toissijainen Vuorovaikutus
windows.controls.confirm Vahvista Vuorovaikutus
windows.controls.drop Tiputa
windows.controls.wepVis Vaihda Aseen Näkyvyyttä
windows.controls.kickVoteY Vote Kick [Kyllä]
windows.controls.kickVoteN Vote Kick [Ei]
windows.controls.kpdVoteY KPD Vote [Kyllä]
windows.controls.kpdVoteN KPD Vote [Ei]
windows.controls.specFree Vaihda Vapaaseen Tilaan
windows.controls.specObj Vaihda Objektitilaan
windows.controls.specFirst Vaihda Ensimmäiseen Persoonaan
windows.controls.specNames Vaihda Pakotetut Nimet
windows.controls.specFocus Vaihda Pelaajaan Keskittymiseen
windows.controls.hidePlayers Piilota Pelaajat
windows.controls.streak0 Tappoputki 1
windows.controls.streak1 Tappoputki 2
windows.controls.streak2 Tappoputki 3
windows.controls.streak3 Tappoputki 4
windows.controls.streak4 Tappoputki 5
windows.controls.taunt0 Päästä Ääni (Psst)
windows.controls.taunt1 Päästä Ääni (Whistle)
windows.controls.taunt2 Päästä Ääni (Fart)
windows.controls.taunt3 Päästä Ääni (Meme)
windows.controls.taunt4 Päästä Ääni (WTF is This?)
windows.controls.taunt5 Päästä Ääni (Right Now!)
windows.controls.prop Valitse Prop (Pidä Painettuna)
windows.controls.propRand Valitsr Satunnainen Prop
windows.controls.propRot Vaihda Rotaatio
windows.controls.propRotR Resetoi Rotaatio
windows.controls.premium0 Noclip
windows.controls.premium1 Jumala-tila
windows.controls.premium2 Tappo
windows.controls.premium3 Smite
windows.controls.reset Nollaa Sijainti (Race & Parkour)
windows.controls.pretend Piilota Peli
windows.host.header Luo Kustomoitu Peli
windows.host.maps Palvelinasetukset
windows.host.cmaps Yhteisöpelit
windows.host.raw Raakapelidata
windows.host.modes Valitse Pelimuodot
windows.host.classes Valitse Hahmot
windows.host.settings Palvelinasetukset
windows.host.preset Esiasetukset
windows.host.preset.load Ladataan Esiasetuksia...
windows.host.preset.name Esiasetuksen nimi
windows.host.start Aloita Peli
windows.host.rehost Aloita Peli (Tuo Pelaajia)
windows.spray.header Valitse Spray
windows.spray.default Vapauta Spray
windows.spray.spin Spray
windows.maps.header Kustomoidut Pelit
windows.maps.none Pelejä Ei Löydetty!
windows.maps.name Pelin Nimi
windows.maps.editor Pelintekijä
windows.maps.publish Julkaise Peli
windows.maps.community Kehittäjäyhteisö
windows.publish.map.header Julkaise tai Päivitä Peli
windows.publish.map.thumbnail Ei Pikkukuvaa
windows.publish.map.paste Liitä Pelitiedot
windows.publish.map.update Julkaise/Päivitä
windows.publish.map.login luodaksesi ja ladataksesi Pelejä!
windows.theatre.header Teatteritila
windows.theatre.record Ota Tallennukset Käyttöön
windows.theatre.key Tallennuksen Näppäin
windows.clans.header Klaanit
windows.clans.members Jäseniä ({0})
windows.clans.requests Liittymispyynnöt
windows.clans.requests.none Ei Liittymispyyntöjä
windows.clans.page [{0}] Klaanisivu
windows.clans.create Luo Klaani
windows.clans.name Klaanin Nimi
windows.clans.view Katso Klaaneja
windows.clans.join Liity Klaaniin
windows.clans.login luodaksesi tai liittyäksesi Klaaniin!
windows.store.header Kauppa
windows.store.spin {0} Spin
windows.store.agree Ostamalla Hyväksyt {0}Käyttöehdot
windows.store.purchase Osta KR:ää
windows.store.voucher Lunasta Koodi
windows.store.voucherinfo Saa Koodeja tietyiltä striimaajilta & tapahtumista!
windows.store.market Kauppapaikka
windows.store.market2 Krunker-Kauppa
windows.store.premium Osta Premium
windows.store.premium2 Krunker Premium
windows.store.buyP Valitse näyttönimi ja paljon muuta ostamalla
windows.store.buy Osta ja myy tuotteita ostamalla
windows.store.wheels Palkintopyörät
windows.store.unlock Avaa uusia aseiden skinejä ja esineitä
windows.store.amount {0} KR:ää käyttäjällä
windows.store.login ostaaksesi ja avataksesi tavaroita kauppapaikalta!
windows.store.purchasing Ostetaan...
windows.store.unboxed {0}{1} unboxasi
windows.store.found {0}{1} löysi
windows.trade.header Kaupankäynti
windows.scope.header Valitse Tähtäin
windows.premium.header Premium
windows.skin.header Valitse Skini
windows.hat.header Valitse Hattu
windows.back.header Valitse Selkäesine
windows.waist.header Valitse Vyötäröesine
windows.face.header Valitse Naama
windows.melee.header Valitse Melee
windows.attach.header Valitse Lisävaruste
windows.reticle.header Valitse Reticle
windows.reticle.unlocked Avautuu tasolla {0}
windows.reticle.req Taso {0} Vaadittu
windows.dye.header Valitse Värjäys
windows.dye.default Oletus-Värjäys
windows.dye.spin Värjäys
windows.changelog.header Muutosloki
windows.mods.community.header Yhteisömodit
windows.mods.community.fav Suosikkisi
windows.mods.community.new Uudet modit
windows.mods.community.hot Trendaavat
windows.mods.community.name Modin Nimi
windows.mods.community.none Modeja ei löydetty
windows.publish.mod.header Julkaise Modi
windows.publish.mod.thumbnail Ei Pikkukuvaa
windows.publish.mod.name Modin Nimi
windows.publish.mod.paste Modin URL
windows.publish.mod.update Julkaise/Päivitä
windows.publish.mod.login luodaksesi ja ladataksesi modeja!
windows.secondary.header Valitse Toissijainen
windows.secondary.unlocked Avattu
windows.secondary.customOnly Vain Custom-Peleissä
windows.secondary.premiumOnly Vain Premiumin Kanssa
windows.secondary.req Taso {0} Vaadittu
windows.advertise.header Mainosta
windows.support.header Tuki
windows.players.header Pelaajalista
windows.players.none Ei pelaajia tällä hetkellä...
windows.join.header Liity
windows.join.code Syötä Pelin URL/Koodi
windows.client.header Client
windows.client.windows Windows-Asentaja
windows.client.mac macOS-Asentaja
windows.client.linux Linux-Asentaja
windows.ranked.header Kilpailullinen
windows.ranked.login pelataksesi kilpailullisia pelejä!
windows.ranked.level Sinun pitää olla vähintään <span color="black">taso {0}</span> pelataksesi kilpailullisia pelejä! Tällä hetkellä olet <span color="black">taso {1}</span>.
windows.chall.header Haasteet
windows.chall.login aloittaaksesi haasteet!
thumbnail.upload.limit Epäonnistui. Pikkukuvan minimi tiedostokoko {0}kb
image.upload.limit Epäonnistui. Kuvan minimi tiedostokoko on {0}kb
thumbnail.upload.error Pikkukuva - Virhe
mod.extracting puretaan {0}/{1}
mod.disabled modit pois päältä
mod.error modi - virhe!
mod.loading ladataan modia...
mod.invalid virheellinen mod-tiedosto!
matchmaker.full TÄYNNÄ
matchmaker.GameFull Peli on täysi.
matchmaker.NoServersMatchQuery PELI PÄIVITTYY
matchmaker.InvalidGameId Peliä ei löytynyt.
matchmaker.NoAvailableServers Serverit ovat Maksimikapasiteetilla
matchmaker.GameIssue Tuntematon Liittymisongelma
adblocker Poista adblock käytöstä saadaksesi ilmaisia Krunkieita.
ad-failed Mainoksen Lataaminen Epäonnistui. Yritä Uudelleen.
streamers.views {0} katsojaa
streamers.none Ei aktiivisia striimejä!
error.extentions Jos sinulla on laajennuksia, poista ne käytöstä
error.seek Etsi uusi peli klikkaamalla
custom.map.upload.limit Epäonnistui. 500 000 Merkin Pelinjulkaisuraja
custom.map.missing Valitse vähintään yksi Peli tai Kartta
custom.mode.missing Valitse vähintään yksi Pelimuoto
custom.class.missing Valitse vähintään yksi Hahmo
server.config.maxPlayers Pelaajia
server.config.minPlayers Pelaajien Vähimmäismäärä
server.config.lives Elämiä
server.config.gameTime Minuutteja
server.config.gravMlt Painovoima
server.config.tickRI Korkea Tickrate
server.config.maxPS 40 Pelaajan Palvelin
server.config.promServ Mainosta Palvelinta
server.config.jumpMlt Hypyn Voimakkuus
server.config.airStrf Suunnanvaihto Ilmassa
server.config.deltaMlt Aikaskaala
server.config.strafeSpd Suunnanvaihdon Nopeus
server.config.fiRat Ampumisnopeus
server.config.reSpd Aseen Lataamisen Nopeus
server.config.slidTime Liu'un Aika
server.config.fixMov Liikekorjaus
server.config.slidSpd Liu'un Nopeus
server.config.healthMlt Elämänpisteiden Kerroin
server.config.impulseMlt Aseimpulssi
server.config.nameTeam1 Tiimin 1 Nimi
server.config.nameTeam2 Tiimin 2 Nimi
server.config.nameTeam3 Tiimin 3 Nimi
server.config.nameTeam4 Tiimin 4 Nimi
server.config.nameTeam5 Tiimin 5 Nimi
server.config.t1Dmg Tiimin 1 Vahinko
server.config.t2Dmg Tiimin 2 Vahinko
server.config.t3Dmg Tiimin 3 Vahinko
server.config.t4Dmg Tiimin 4 Vahinko
server.config.t5Dmg Tiimin 5 Vahinko
server.config.selTeam Valitse Tiimi
server.config.allowSpect Katsotaan
server.config.killRewards Tappopalkinnot
server.config.headshotOnly Ainoastaan Pääosumat
server.config.noSecondary Poista Toissijaiset Käytöstä
server.config.noStreaks Poista Tappoputket Käytöstä
server.config.canSlide Liukuminen
server.config.wallJP Seinähypyn Voimakkuus
server.config.autoJump Automaattinen Hyppääminen
server.config.bDrop Luotien Putoaminen
server.config.thirdPerson Kolmas Persoona
server.config.requireCaptcha Vaadi Captchaa
server.config.nameTags Piilota Pelinimet
server.config.kCams Tappokamerat
server.config.aAnon Salli Anonyymiys
server.config.noReload Ei Aseen Lataamista
server.config.private Yksityinen
server.config.krRewards KR-Palkinnot
server.config.forceDefault Pakota Oletusasetukset
server.config.warmupTime Lämmittelyaika
server.config.gamRounds Kierroksia
server.config.objtvTime Tavoitteen Kesto
server.config.forceSpawn Automaattisen uudelleensyntymisen aika
server.config.intermTmr Odotusaika (s)
server.config.scoreLimit Pisteraja
server.config.keepTScore Pidä Tiimin Pisteet
server.config.forceC Pakota Pelimuodon Hahmot
server.config.logTim Lokiaika (Race, Parkour)
server.config.hitBoxPad Osumalaatikoiden Pehmennys
server.config.hpRegen HP:n Regenerointi
server.config.disableB Poista Reunat
server.config.throwMel Heitettävät Meleet
server.config.noCosm Estä Kosmeettiset Tavarat
server.config.tstCmp Kokeile Pelimuotoa
server.config.limitClasses Rajoita Hahmoja
server.config.tmSize Tiimin Koko
server.config.fallDmg Putoamisvahinko
server.config.fallDmgThr Putoamisvahingon Kynnys
server.config.noDraws Ei Tasapelejä
server.config.bstOfR Paras kolmesta
server.config.chrgWeps Ladattavat Aseet
social.login Kirjaudu/Rekisteröidy
social.login.failed Kirjautuminen Epäonnistui
social.login.buy Kirjaudu Sisään Ostaaksesi
social.login.sell Kirjaudu Sisään Myydäksesi
social.login.gift Kirjaudu Sisään Lahjoittaaksesi
social.login.proccessing Kirjaudutaan sisään...
social.since Since
social.user.maps Käyttäjän pelit
social.user.mods Käyttäjän modit
social.maps.none Pelejä Ei Löydetty...
social.mods.none Modeja Ei Löydetty...
social.market.price.low Alin Hinta
social.market.price.list Myyntihinta
social.market.searching Etsitään
social.market.listed Laitettu myyntiin {0} sitten
social.market.list Laita Esine Myyntiin
social.market.unlist Poista Esine Myynnistä
social.market.unlisting Poistetaan Esinettä Myynnistä...
social.market.posting Julkaistaan Esinettä...
social.market.req taso {0} vaadittu
social.market.cant Ei Ole Varaa
social.market.purchase Osta
social.market.fee Ei Myyntiinlaittomaksua
social.market.failed Epäonnistui. Yritä Uudelleen
social.market.head Pään Esine
social.market.back Selkäesine
social.market.waist Vyötäröesine
social.market.face Naama
social.market.shoe Kenkäesine
social.market.pet Lemmikkiesine
social.market.wrist Ranne-Esine
social.market.melee Melee-Esine
social.market.spray Spray-Esine
social.market.dye Värjäys
social.market.collectible Keräiltävä Esine
social.market.none Esineitä Ei Löydetty
social.market.login Sinun Pitää Olla Kirjautunut Sisään.
social.market.grace valmiina {0} jälkeen
social.market.sort Suodata/Lajittele
social.market.filter Etsi
social.market.sort2 Lajittele
social.market.price.high Korkein Hinta
social.trading.submitting Tarjousta Lähetetään...
social.market.gift.amount Lahjasumma
social.market.fee.gift Ei Maksua
social.market.gift Lahjoita KR:ää
social.market.gift.msg Jätä viesti
respawn.auto Uudelleensynnytään {0} sekunnin kuluttua
custom.host.ranked Et voi luoda peliä ollessasi kilpailullisessa
funds.insufficient Ei ole varaa
funds.out Sinulla ei ole tarpeeksi Krunkieita.
custom.passcode.long Salasana liian pitkä
custom.passcode.input Syötä palvelimen salasana liittyäksesi
custom.tryagain Yritä uudelleen.
purchase.kr.success {0} KR:ää lisätty käyttäjällesi
limited.purchase.ask Oletko varma että haluat ostaa [{0}]:n {1} KR:llä?
limited.purchase.success {0} on nyt lisätty  to your tavaraluetteloosi
pretend.exit paina [{0}] uudelleen poistuaksesi
settings.loaded Asetukset Ladattu
settings.loaded.none Asetuksia ei löydetty
account.login.required Pitää olla kirjautuneena sisään
account.banned.hack Tämä käyttäjä on saanut porttikiellon huijaamisesta.
account.banned.appeal Jos haluat valittaa, ota yhteyttä:
account.delete Voit pyytää käyttäjäsi poistamista, jos et hylkää pyyntöäsi 14 päivän kuluessa, käyttäjäsi ja kaikki muu siihen liittyvä tieto poistetaan pysyvästi.
account.delete.request Oletko varma, että haluat lähettää käyttäjäpoistopyynnön?
account.delete.cancel Oletko varma, että haluat hylätä käyttäjäpoistopyynnön?
spin.free.available ILMAISTA KR:ÄÄ SAATAVILLA!
spin.free.claim Tämä spin on lunastettavissa kerran tunnissa ottelun päätyttyä.
spin.free.disable Nämä popupit voi ottaa pois käytöstä "General Settings"-osiosta.
mod.reset.0 Tulee voimaan uudelleensyntymisen jälkeen.
mod.reset.1 Kartan Objektit eivät nollaudu
windows.profile.picture Vaihta profiilikuva
stats.assists Avustukset
stats.nukes Ydinpommit
stats.melee Melee-tapot
stats.fistkills Nyrkkitapot
stats.thrownkills Heittotapot
stats.headshots Pääosumat
stats.wallbangs Wallbangit
stats.crouches Kyykyt
stats.sprays Sprayta Maalattu
stats.kpd Hyödylliset Ilmiannot
windows.host.default.settings Oletusasetusten ottaminen käyttöön mahdollistaa pelaajan edistymisen ja huipputulosten tallentamisen
windows.host.default.commands *Palvelinomistajan komennot ovat pois päältä tällä modella: /noclip /god /smite etc
windows.host.rewards Ottaessasi käyttöön KR-Palkinnot peli pakottaa palvelimen käyttämään Default-asetuksia & antaa pelaajien saada KR:ää kartan rahastosta.
windows.store.wheels.sub Kokeile onneasi, pyöräytä ja saa mahdollisuus voitaa uusia skinejä!
windows.store.currency * Hinnat USD:ssä voidaan muuntaa paikalliseksi valuutaksi maksupalveluntarjoajan tai pankin määrittelemällä valuuttakurssilla
windows.store.creator Harkitse suosikkisisällöntuottajasi tukemista käyttämällä luojakoodia
windows.chall.customs Haasteita ei voi suorittaa mukautetuissa otteluissa
twitch.link Oletko varma, että haluat linkittää Twitch-käyttäjäsi?
twitch.unlink Oletko varma, että haluat poistaa Twitch-käyttäjäsi linkityksen?
windows.clan.leave Oletko varma, että haluat poistua klaanista?
windows.clan.delete Oletko varma, että haluat poistaa klaanisi?
windows.clan.kick Oletko varma, että haluat poistaa {0}:nklaanista?
windows.store.premium.purchase Oletko varma, että haluat ostaa {0} päivää Premiumia?
windows.store.premium.gift Oletko varma, että haluat lahjoittaa '{0}' {1} päivää Premiumia?
windows.playerlist.ban Oletko varma, että haluat antaa porttikiellon Käyttäjälle {0}?
windows.playerlist.flag Oletko varma, että haluat merkata käyttäjän {0}?
windows.playerlist.unflag Oletko varma, että haluat poistaa merkinnän käyttäjältä {0}?
windows.playerlist.punish Haluatko varmasti ryhtyä toimiin käyttäjään {0}?
item.view.open Avaa Mallin Tarkastelija
item.view.more Lisätietoja
item.sell.cant Esinettä ei voi myydä.
item.sell.cant.krunker Esinettä ei voi myydä Krunkerissa.
item.sell.quick Oletko varma, että haluat nopeamyydä esineen {0}?
item.sell.destroy Haluatko varmasti Tuhota esineen {0}?
social.profile.stats Kokonaistilastot
social.profile.class Hahmotilastot
social.profile.sales Myynnit
social.profile.sales.market Market-myynnit
social.profile.skins Skinejä [{0}]
social.profile.skins.none Ei julkaistuja skinejä tällä käyttäjällä
social.profile.games Pelejä [{0}]
social.profile.games.none Ei julkaistuja pelejä tällä käyttäjällä
social.profile.assets Assetit [{0}]
social.profile.assets.none Ei julkaistuja assetteja tällä käyttäjällä
social.profile.mods Modeja [{0}]
social.profile.mods.none Ei julkaistuja modeja tällä käyttäjällä
social.search Hakutulokset
social.market.stats.rare Harvinaisimmat esineet
social.market.stats.circulation Liikkeellä
social.market.stats.units yksikköä maailmanlaajuisesti
social.trades.in Tulevat
social.trades.in.head Tulevat tarjoukset ({0})
social.trades.out Lähtevät
social.trades.out.head Lähtevät tarjoukset ({0})
social.trades.history Historia
social.trades.history.head Vaihtokauppahistoria ({0})
social.trades.decline Hylkää kaikki
social.trades.message HYVÄKSY VAIHTOKAUPPAVIESTIT ASETUKSISSA
social.trades.warning Et saa yhtäkään esinettä tässä vaihtokaupassa!
social.trades.accept Oletko varma, että haluat hyväksyä tämän vaihtokaupan?
stats.airdrops KR-Paketit
stats.airdrops.stolen Airdroppeja Varastettu
settings.quality.flapAnim Tarkkuuskiväärin Läpän Animaatio
chall.name.hs Saa {0} headshottia {1} 
​chall.name.ns Saa {0} no-scopea {1} chall.name.ls Saa {0} legshottia {1} 
chall.name.tmk Saa {0} puukonheittotappoa {1} 
chall.name.fb Saa {0} ensimmäistä tappoa {1} 
chall.name.mk Saa {0} melee-tappoa {1} 
chall.name.fk Saa {0} nyrkkitappoa {1} 
chall.name.qs Saa {0} quickscopea {1} 
chall.name.wb Saa {0} wallbangia {1} ​
chall.name.airk Saa {0} mid-air-tappoa {1} 
chall.name.lns Saa {0} longshottia {1} 
​chall.name.tk Saa {0} turbo-tappoa {1} 
chall.name.drk Saa {0} drifti-tappoa {1} 
chall.name.kstk Saa {0}:n tappoputki {1} 
​chall.name.w Voita {0} ottelua 
​chall.name.d Kuole {0} kertaa 
chall.name.score Saa {0} pistettä 
chall.name.dmg Tee {0} vahinkoa {1} 
chall.name.bzk Saa {0} buzz-tappoa {1} 
chall.name.cc Saa {0} läheltä piti-tappoa {1} 
chall.name.rk Saa {0} kostotappoa {1} 
`;
